...
// Declaring variables
uint8_t payloadLength = ACCESS_POINTS * SF_WIFI_BSSID_SIZE;
...
void loop()
{
  ...
  while (millis() < timeout && bssidNumber < ACCESS_POINTS)
  {
    line = readLine();
    ...
    if (bssid && strlen(bssid) == SF_WIFI_BSSID_SIZE * 3 - 1)
    {
      ...
      char *b = strtok(bssid, ":");
      for (bssidByte = 0; bssidByte < SF_WIFI_BSSID_SIZE && b; bssidByte++, b = strtok(NULL, ":"))
      {
