...
<body>
  ...
  $.get("/generator", function(data) {
    var uluru = {
      lat: parseFloat(JSON.stringify(data.lat)),
      lng: parseFloat(JSON.stringify(data.lng))
    };
  ...
  <script async defer
  src="https://maps.googleapis.com/maps/api/js?key=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx&callback=initMap">
  </script>
  <script src="http://code.jquery.com/jquery-2.0.3.min.js"></script>
</body>
</html>
...
Stop the server and start it again. Let's go to http://localhost:8080. You should see a fancy map with the marker on your current location.

<div align="center">
<img src="../img/geolocation.png" width="750"/>
</div>
The messages that you receive in the terminal should also look like this:

<div align="center">
<img src="../img/msg_wifi_localization.png" width="750"/>
</div>

## Troubleshooting

In case the map is load before receiving any WiFi address the API call could fail or send back a default location. Same trouble if only one or two addresses are received. The application becomes more accurate if the API call is made providing at least three WiFi addresses.
